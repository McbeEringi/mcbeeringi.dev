...
Object.entries({
	...
	fetch:async e=>console.log(await(async (r,x)=>x?(x.slice(0,6)=='bytes='&&(x=x.slice(6).split(/,\s*/)).length==1?(
		console.log(`RANGE ${x}`),
		x=x.split('-'),
	...
	),e.respondWith(await(async (r,x)=>x?(x.slice(0,6)=='bytes='&&(x=x.slice(6).split(/,\s*/)).length==1?(
		console.log(`RANGE ${x}`),
		x=x.split('-'),
		r=await r.arrayBuffer(),
		x=[
...
}).forEach(([i,x])=>self.addEventListener(i,e=>(console.log(i,e),e.waitUntil(x(e)))));
